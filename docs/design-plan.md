...
- M3 修复管线 + Coordinator 接入真实 `tools/*` / `ctx.subagents` — 已完成（scavenge/flatten/truncation/storm + planner/executor 子 Agent）
- M4 配置兼容（reasonix.toml → profile）+ 基准验证 — 已完成（`src/config/` + `src/benchmark/`，缓存命中率 ≥90%、成本 ≤1.1×、成功率不回退）
- M5 默认切换 + 回滚通道冻结
